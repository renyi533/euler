...
class SignedUnsupervisedModel(base.Model):
  ...
  def to_sample(self, inputs):
    ...
    real_neg_cnt = self.num_negs/10
    real_neg_cnt = min(max(real_neg_cnt, 1), 10)
    print('sample %d random samples(share=%r)  and %d real neg samples' % (self.num_negs, self.share_negs, real_neg_cnt))
    real_negs = euler_ops.sample_neighbor(inputs, self.neg_edge_type, real_neg_cnt,
                                    self.max_id + 1)[0]
    pos_negs = tf.concat([pos_negs, real_negs], axis=1)

    return src, pos, pos_negs
...
class SignedUnsupervisedModelV2(base.Model):
  ...
  def __init__(self,
               ...
               *args,
               **kwargs):
    super(SignedUnsupervisedModelV2, self).__init__(*args, **kwargs)
    self.node_type = node_type
    self.edge_type = edge_type
    self.neg_edge_type = neg_edge_type
    self.max_id = max_id
